...
def _sheet(ws, headers, rows):
    ...
    ws.freeze_panes = "A2"
    if headers: ws.auto_filter.ref = ws.dimensions
...
def install_postrollback_export(app):
    ...
    def export_postrollback():
        ...
        if not url: return jsonify({"error":"اتصال PostgreSQL غير مضبوط"}),503
        try:
    ...
    def api_update_vip_customer(customer_id):
        ...
        data=request.get_json(silent=True) or {}
        try:
            vip_customers.ensure_schema(); customer=vip_customers.update_customer(customer_id,data.get("name"),data.get("phone"),data.get("offer_product"),data.get("offer_price"))
            if customer is None:return jsonify({"error":"العميل غير موجود"}),404
    ...
    def api_vip_customer_status(customer_id):
        ...
        data=request.get_json(silent=True) or {}
        try:
            vip_customers.ensure_schema(); customer=vip_customers.set_status(customer_id,str(data.get("status") or ""))
            if customer is None:return jsonify({"error":"العميل غير موجود"}),404
            return jsonify({"customer":customer})
        except ValueError as exc:return jsonify({"error":str(exc)}),400
        except Exception as exc:
