...
function switchView(v){const current=document.querySelector(".view.active")?.id?.replace(/^view-/,"");document.querySelectorAll(".view").forEach(x=>x.classList.remove("active"));document.querySelectorAll(".nav-btn[data-view]").forEach(x=>x.classList.remove("active"));document.getElementById(`view-${v}`)?.classList.add("active");document.querySelector(`.nav-btn[data-view="${v}"]`)?.classList.add("active");if(current===v)return;if(v==="dashboard")loadDashboard();if(v==="orders")loadOrders();if(v==="backups")loadBackups();if(v==="message-templates")loadMessageTemplates();if(v==="shortages")window.dailyShortages?.open?.();if(v==="whatsapp"){loadMessageTemplates();loadShortages();loadWaCustomers();}if(v==="whatsapp_legacy_never"){loadShortages();loadWaCustomers()}if(v==="new-order")document.querySelector('[name="order_date"]').value ||= todayISO()}
function initNav(){
...
function cancelOrder(id){
  const reason=window.prompt("سبب إلغاء الطلب؟\n\nمثال: العميل رفض / العميل غير متجاوب / سبب آخر");
  if(reason===null) return;
  const note=String(reason||"").trim();
  if(!note){toast("يجب كتابة سبب الإلغاء","error");return;}
  openConfirm(`تأكيد إلغاء الطلب بسبب: ${note}`,async()=>{
    try{
      await apiFetch(`/api/orders/${id}/cancel`,{method:"POST",body:JSON.stringify({note})});
      toast("تم إلغاء الطلب وتسجيل السبب");
...
function renderShortageOrders(){const c=document.getElementById('shortages-orders-list');if(!shortageOrders.length){c.innerHTML='<div class="empty-state">لا توجد طلبات ناقصة حاليًا ✅</div>';document.getElementById('shortages-message').value=buildShortageMessage([],document.getElementById('shortages-mode').value);return}c.innerHTML=shortageOrders.map((o,i)=>{const items=o.Shortage_Items||[];const products=items.map(x=>`${esc(x.Product_Name)} × ${x.Quantity}`).join('<br>');return `<div class="shortage-order-card"><label class="shortage-order"><input type="checkbox" class="shortage-check" data-index="${i}" checked><div class="shortage-order-content"><div class="shortage-order-id">طلب #${esc(o.Order_ID)}</div><div class="shortage-customer">${esc(o.Customer_Name)}</div><div class="shortage-phone">${esc(o.Phone||'رقم الهاتف غير مسجل')}</div><div class="shortage-products">${products}</div></div></label></div>`}).join('');c.querySelectorAll('.shortage-check').forEach(x=>x.onchange=updateShortageMessage);updateShortageMessage()}
