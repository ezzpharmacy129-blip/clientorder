...
function switchView(v){document.querySelectorAll(".view").forEach(x=>x.classList.remove("active"));document.querySelectorAll(".nav-btn[data-view]").forEach(x=>x.classList.remove("active"));document.querySelectorAll(".nav-dropdown-toggle").forEach(x=>x.classList.remove("active"));document.querySelectorAll("[data-nav-dropdown]").forEach(x=>x.classList.remove("open"));document.querySelectorAll("[data-shortages-filter]").forEach(x=>x.classList.remove("active"));document.getElementById(`view-${v}`)?.classList.add("active");document.querySelector(`.nav-btn[data-view="${v}"]`)?.classList.add("active");if(v==="dashboard")loadDashboard();if(v==="orders")loadOrders();if(v==="backups")loadBackups();if(v==="message-templates")loadMessageTemplates();if(v==="shortages")window.dailyShortages?.load?.();if(v==="whatsapp"){loadMessageTemplates();loadShortages();loadWaCustomers();}if(v==="whatsapp_legacy_never"){loadShortages();loadWaCustomers()}if(v==="new-order")document.querySelector('[name="order_date"]').value ||= todayISO()}
function initNav(){
