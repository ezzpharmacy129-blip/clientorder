# -*- coding: utf-8 -*-
"""Allow an order to be explicitly reset to 'بانتظار التوفر' on the local Excel backend.

CloudDB/PostgreSQL has its own native availability implementation, so this
compatibility patch must never wrap CloudDB with Excel-specific internals.
"""


def install_pending_availability_fix(db):
    # This patch relies on Excel workbook internals (_load, worksheet mutation,
    # atomic save, etc.). Never install it on the PostgreSQL backend.
    ...
    def patched(order_id, item_updates, available_date=None, user="موظف"):
        ...
        statuses = [
            str(x.get("availability_status") or "").strip()
            for x in updates
        ...
        if statuses and all(s == "بانتظار التوفر" for s in statuses):
            with _lock:
                ...
                db._append_log(
                    ...
                    old,
                    STATUS_PENDING,
                    "تم إرجاع الطلب إلى بانتظار التوفر",
                    user,
                )
