...
function customerShortageRows(orders){
  ...
  (orders||[]).forEach(o=>{
    ...
    if(pending.length){
      pending.forEach(i=>rows.push({type:"customer",orderId:o.Order_ID,customer:o.Customer_Name,phone:o.Phone,product:i.Product_Name,quantity:i.Quantity||1,date:o.Order_Date,status:"بانتظار التوفر",itemId:i.Item_ID||""}));
    }else if(o.Status==="بانتظار التوفر"){
      rows.push({type:"customer",orderId:o.Order_ID,customer:o.Customer_Name,phone:o.Phone,product:o.Product_Name||"—",quantity:o.Quantity||1,date:o.Order_Date,status:"بانتظار التوفر",itemId:""});
    }
  });
  return rows;
}
function pharmacyShortageRows(items){
  return (items||[]).map(x=>({type:"pharmacy",shortageId:x.shortage_id,orderId:"—",customer:"—",phone:"—",product:x.product_name,quantity:x.quantity,date:x.created_at,status:x.status==="available"?"تم التوفير":"بانتظار التوفير",note:x.note,createdBy:x.created_by}));
}
function dailyShortageTypeLabel(type){
  return type==="pharmacy" ? '<span class="shortage-type shortage-type-pharmacy">الصيدلية</span>' : '<span class="shortage-type shortage-type-customer">عميل</span>';
}
function dailyShortageStatus(row){
  if(row.type==="pharmacy") return '<span class="status-badge '+(row.status==="تم التوفير"?"status-picked":"status-pending")+'">'+esc(row.status)+'</span>';
  return badge(row.status);
}
function dailyShortageRows(){
  if(dailyShortagesFilter==="pharmacy"){
    const limit=pharmacyShortagesLimit==="all"?dailyShortagesCache.pharmacy.length:Number(pharmacyShortagesLimit)||20;
    return dailyShortagesCache.pharmacy.slice(0,limit);
  }
...
function renderDailyShortages(){
  ...
  const labels={all:"الكل",customer:"طلبات العملاء",pharmacy:"نواقص الصيدلية"};
  title.textContent="📦 "+(labels[dailyShortagesFilter]||"النواقص");
  limitWrap?.classList.toggle("hidden",dailyShortagesFilter!=="pharmacy");
  const rows=dailyShortageRows();
  ...
  subtitle.textContent=dailyShortagesFilter==="pharmacy"?"النواقص المسجلة على مستوى الصيدلية فقط.":dailyShortagesFilter==="customer"?"المنتجات غير المتوفرة المرتبطة بطلبات العملاء.":"جميع النواقص الحالية: طلبات العملاء ونواقص الصيدلية.";
  summary.textContent=dailyShortagesFilter==="pharmacy"&&pharmacyShortagesLimit!=="all"?"عرض "+rows.length+" من أصل "+total+" نقص":"عدد النتائج: "+rows.length;
  if(!rows.length){body.innerHTML='<tr><td colspan="9" class="empty-state">لا توجد نواقص ضمن هذا التصنيف ✅</td></tr>';return;}
  body.innerHTML=rows.map(r=>{
    let action;
    if(r.type==="pharmacy"){
      action=r.status==="تم التوفير"
        ? '<button class="btn btn-secondary btn-sm daily-shortage-undo" data-id="'+esc(r.shortageId)+'">↩️ إعادة</button>'
        : '<button class="btn btn-primary btn-sm daily-shortage-available" data-id="'+esc(r.shortageId)+'">تم التوفير</button>';
    }else{
      action='<button class="btn btn-outline btn-sm daily-shortage-detail" data-id="'+esc(r.orderId)+'">التفاصيل</button>';
    }
    return '<tr><td>'+dailyShortageTypeLabel(r.type)+'</td><td><strong>'+esc(r.orderId)+'</strong></td><td>'+esc(r.customer)+'</td><td dir="ltr">'+esc(r.phone)+'</td><td><strong>'+esc(r.product)+'</strong>'+(r.note?'<div class="daily-shortage-note">'+esc(r.note)+'</div>':"")+'</td><td>'+esc(r.quantity)+'</td><td>'+fmtDate(r.date)+'</td><td>'+dailyShortageStatus(r)+'</td><td><div class="daily-shortage-actions">'+action+'</div></td></tr>';
  }).join("");
...
async function loadDailyShortages(){
  try{
    const results=await Promise.all([apiFetch("/api/orders"),apiFetch("/api/pharmacy-shortages")]);
    dailyShortagesCache.customer=customerShortageRows(results[0].orders||[]);
    dailyShortagesCache.pharmacy=pharmacyShortageRows(results[1].shortages||[]);
    renderDailyShortages();
  }catch(e){toast(e.message,"error");}
}
...
function openPharmacyShortageModal(){
  const m=document.getElementById("pharmacy-shortage-modal"),f=document.getElementById("pharmacy-shortage-form");
  if(!m||!f)return;
  f.reset();f.quantity.value=1;m.classList.remove("hidden");m.setAttribute("aria-hidden","false");setTimeout(()=>f.product_name.focus(),0);
}
function closePharmacyShortageModal(){
  const m=document.getElementById("pharmacy-shortage-modal");
...
async function submitPharmacyShortage(e){
  ...
  btn.disabled=true;
  try{
    await apiFetch("/api/pharmacy-shortages",{method:"POST",body:JSON.stringify(payload)});
    toast("تمت إضافة نقص الصيدلية");
    closePharmacyShortageModal();
    dailyShortagesFilter="pharmacy";
    localStorage.setItem("ezz_daily_shortages_filter","pharmacy");
    document.querySelectorAll("[data-shortages-filter]").forEach(x=>x.classList.toggle("active",x.dataset.shortagesFilter==="pharmacy"));
    await loadDailyShortages();
...
function initDailyShortages(){
  ...
  const s=document.getElementById("pharmacy-shortages-limit");
  if(s)s.value=["20","30","all"].includes(pharmacyShortagesLimit)?pharmacyShortagesLimit:"20";
...
function buildShortageMessage(selected,mode='orders'){
  ...
  let body='';
  if(mode==='grouped'){
    const grouped={}; orders.forEach(o=>(o.Shortage_Items||[]).forEach(i=>{const k=String(i.Product_Name||'').trim().toLowerCase();const r=grouped[k]||(grouped[k]={name:i.Product_Name,qty:0,orders:0});r.qty+=Number(i.Quantity)||0;r.orders++;}));
    body=Object.values(grouped).sort((a,b)=>b.qty-a.qty).map((x,i)=>`${i+1}. ${x.name} — إجمالي المطلوب: ${x.qty} (${x.orders} طلب)`).join('\n');
  }else{
    body=orders.map(o=>`• ${o.Customer_Name} — ${o.Order_ID}\\n${(o.Shortage_Items||[]).map(i=>`  - ${i.Product_Name} × ${i.Quantity}`).join('\n')}`).join('\n');
  }
